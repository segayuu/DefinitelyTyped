...
const coroutine2 = Bluebird.coroutine(function*(a: number, b: string) {
	foo = yield fooProm;
	return barProm;
});
barProm = coroutine2(5, "foo");

// TODO: Eliminate type arguments for generator functions that are passed different types.
const coroutine3 = Bluebird.coroutine<Baz, Foo | Bar, [number, string]>(function*(a: number, b: string) {
	foo = (yield fooProm) as Foo;
	bar = (yield barProm) as Bar;
	return baz;
});
bazProm = coroutine3(5, "foo");
